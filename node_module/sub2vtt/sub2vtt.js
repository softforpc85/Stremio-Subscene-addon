...
class sub2vtt {
    constructor(url, opts = {}) {
        let { proxy, type , episode} = opts;
        this.url = url;
        this.proxy = proxy || opts.proxy || {};
        this.data = null;
        this.size = null;
        this.error = null;
        this.type = type || opts.type || {responseType: "buffer"};
        this.client = null;
        this.episode = episode || opts.episode || null;
    }

    async GetData() {
        let res = await got.get(this.url, {
            responseType: "buffer"
          });

        if (!res || !res.status == "200" || !res.headers) throw "Loi GetData"
    ...
    async getSubtitleMoi(url = String) {
        try {
            // checking the link
            let downUrl = url;
            console.log("downUrl :", url)
            if (!downUrl)throw "Unexpected Error"
            let res = await got.get(downUrl, {
              responseType: "buffer"
            })
            ...
            //console.log("Lay header type :", res.headers);
            //console.log("Lay header size :", this.size);
    ...
    static gerenateUrl(url = String, opts) {
        ...
        //let {proxy, type } = opts; //Type : movies, series ...
       ...
       if (this.proxy) {
        ...
        if (this.type) data.append("type", type); 
